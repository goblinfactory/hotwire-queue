...
namespace Icodeon.Hotwire.Framework.Modules
{
    ...
    public class EndpointRequestMatcher
    {
        ...
        public EndpointMatch MatchRequestOrNull(string httpMethod, Uri requestUrl)
        {
            ...
            if (_configuration.ExclusiveUse && endpoint == null && requestUrl.LocalPath.TrimStart(new char[]{'\\','/'}).StartsWith(_configuration.RootServiceName))
            {
                throw new HttpModuleException(HttpStatusCode.NotFound, "Endpoint service is configured as 'exclusiveUse' meaning that requests must match at least one endpoint. No endpoints matching '" + requestUrl + "' found.");
            }
